#!/bin/bash
# Create ML Domain GCS Bucket and BigQuery Dataset
#
# Creates the ML domain infrastructure:
# - GCS bucket: ml-models-store (production)
# - GCS bucket: ml-models-store-test (test)
# - BigQuery dataset: ml_predictions
#
...
BUCKET_NAME="${ML_GCS_BUCKET:-ml-models-store}"
TEST_BUCKET_NAME="${ML_GCS_BUCKET_TEST:-ml-models-store-test}"
DATASET_NAME="${ML_BIGQUERY_DATASET:-ml_predictions}"

echo "🚀 Creating ML domain infrastructure..."
...
if bq ls -d $PROJECT_ID:$DATASET_NAME &> /dev/null; then
    ...
    bq mk --dataset \
        --location=$REGION \
        --description="ML domain - model predictions and training data" \
...
echo "🎉 ML domain infrastructure created successfully!"
echo ""
echo "📝 Next steps:"
echo "   1. Update .env files with:"
echo "      ML_GCS_BUCKET=$BUCKET_NAME"
echo "      ML_GCS_BUCKET_TEST=$TEST_BUCKET_NAME"
echo "      ML_BIGQUERY_DATASET=$DATASET_NAME"
